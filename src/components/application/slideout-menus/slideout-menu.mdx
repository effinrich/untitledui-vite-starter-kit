import { Canvas, Meta, ArgTypes } from '@storybook/blocks'
import * as SlideoutMenuStories from './slideout-menu.stories'

<Meta of={SlideoutMenuStories} />

# SlideoutMenu

The `SlideoutMenu` component is a reusable UI element.

## Usage

### Basic Example

<Canvas of={SlideoutMenuStories.Default} />

```tsx
<SlideoutMenu>
  Content
</SlideoutMenu>
```

## Props

<ArgTypes of={SlideoutMenuStories} />

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `children` | `ReactNode \| ((children: AriaModalRenderProps & { close: () => void` | `-` | - |


## Accessibility

The `SlideoutMenu` component follows WAI-ARIA guidelines:

- Supports keyboard navigation
- Includes proper ARIA attributes
- Compatible with screen readers

### Keyboard Interactions

| Key | Description |
|-----|-------------|
| `Tab` | Moves focus to the component |
| `Enter` | Activates the component |
| `Space` | Activates the component |

## Composition

### With Other Components

```tsx
<div className="flex flex-col gap-4">
  <SlideoutMenu>
    First item
  </SlideoutMenu>
  <SlideoutMenu>
    Second item
  </SlideoutMenu>
</div>
```

## Responsive Design

Use CSS media queries for responsive behavior:

```tsx
<SlideoutMenu
  
  style={{ width: '100%' }}
  className="responsive-component"
>
  Responsive SlideoutMenu
</SlideoutMenu>

<style>
  @media (min-width: 768px) {
    .responsive-component {
      width: auto;
      
    }
  }
</style>
```

## Best Practices

1. **Accessibility**: Always provide meaningful content or aria-label
4. **Composition**: Combine with other components using Stack or Flex for layouts
5. **Performance**: Avoid creating new callback functions on each render

## Related Components

- See your design system documentation for related components
