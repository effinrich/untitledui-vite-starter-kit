import { Canvas, Meta, ArgTypes } from '@storybook/blocks'
import * as AppStoreButtonsStories from './app-store-buttons.stories'

<Meta of={AppStoreButtonsStories} />

# AppStoreButtons

The `AppStoreButtons` component is a reusable UI element.

## Usage

### Basic Example

<Canvas of={AppStoreButtonsStories.Default} />

```tsx
<AppStoreButtons>
  Content
</AppStoreButtons>
```

## Props

<ArgTypes of={AppStoreButtonsStories} />

This component accepts standard HTML attributes.

## Accessibility

The `AppStoreButtons` component follows WAI-ARIA guidelines:

- Supports keyboard navigation
- Includes proper ARIA attributes
- Compatible with screen readers

### Keyboard Interactions

| Key | Description |
|-----|-------------|
| `Tab` | Moves focus to the component |
| `Enter` | Activates the component |
| `Space` | Activates the component |

## Composition

### With Other Components

```tsx
<div className="flex flex-col gap-4">
  <AppStoreButtons>
    First item
  </AppStoreButtons>
  <AppStoreButtons>
    Second item
  </AppStoreButtons>
</div>
```

## Responsive Design

Use CSS media queries for responsive behavior:

```tsx
<AppStoreButtons
  
  style={{ width: '100%' }}
  className="responsive-component"
>
  Responsive AppStoreButtons
</AppStoreButtons>

<style>
  @media (min-width: 768px) {
    .responsive-component {
      width: auto;
      
    }
  }
</style>
```

## Best Practices

1. **Accessibility**: Always provide meaningful content or aria-label
4. **Composition**: Combine with other components using Stack or Flex for layouts
5. **Performance**: Avoid creating new callback functions on each render

## Related Components

- See your design system documentation for related components
