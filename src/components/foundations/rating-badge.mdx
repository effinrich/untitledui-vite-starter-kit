import { Canvas, Meta, ArgTypes } from '@storybook/blocks'
import * as RatingBadgeStories from './rating-badge.stories'

<Meta of={RatingBadgeStories} />

# RatingBadge

The `RatingBadge` component is a reusable UI element.

## Usage

### Basic Example

<Canvas of={RatingBadgeStories.Default} />

```tsx
<RatingBadge>
  Content
</RatingBadge>
```

## Props

<ArgTypes of={RatingBadgeStories} />

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `title` | `string` | `-` | - |
| `subtitle` | `string` | `-` | - |
| `rating` | `number` | `-` | - |
| `theme` | `"light" \| "dark"` | `-` | - |


## Accessibility

The `RatingBadge` component follows WAI-ARIA guidelines:

- Supports keyboard navigation
- Includes proper ARIA attributes
- Compatible with screen readers

### Keyboard Interactions

| Key | Description |
|-----|-------------|
| `Tab` | Moves focus to the component |
| `Enter` | Activates the component |
| `Space` | Activates the component |

## Composition

### With Other Components

```tsx
<div className="flex flex-col gap-4">
  <RatingBadge>
    First item
  </RatingBadge>
  <RatingBadge>
    Second item
  </RatingBadge>
</div>
```

## Responsive Design

Use CSS media queries for responsive behavior:

```tsx
<RatingBadge
  
  style={{ width: '100%' }}
  className="responsive-component"
>
  Responsive RatingBadge
</RatingBadge>

<style>
  @media (min-width: 768px) {
    .responsive-component {
      width: auto;
      
    }
  }
</style>
```

## Best Practices

1. **Accessibility**: Always provide meaningful content or aria-label
4. **Composition**: Combine with other components using Stack or Flex for layouts
5. **Performance**: Avoid creating new callback functions on each render

## Related Components

- See your design system documentation for related components
